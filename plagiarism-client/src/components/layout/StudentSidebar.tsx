...
import {
  Home, Upload, FileText, User, LogOut, ChevronLeft,
} from "lucide-react";
...
const navItems = [
  ...
  { heading: "Assignments" },
  { label: "Submit Assignment", icon: Upload, path: "/student/submit" },
  { label: "My Results", icon: FileText, path: "/student/results" },
  { heading: "Account" },
        ...
        <div className="mb-4">
          {!collapsed && user && (
            <div className="mb-3 rounded-lg bg-sidebar-accent/30 px-3 py-2">
              ...
              <p className="text-[10px] text-sidebar-foreground/50">{user.regNo}</p>
            </div>
          )}
          <button
            onClick={logout}
